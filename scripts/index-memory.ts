...
import type { SourceType, MemoryItem } from '../src/types/memory.js';
...
async function main() {
  ...
  try {
    const { ClaudeMdSource } = await import('../src/parsers/claude-md-source.js');
    registry.register(new ClaudeMdSource());
  } catch { /* not yet implemented */ }

  try {
    const { HistorySource } = await import('../src/parsers/history-source.js');
    registry.register(new HistorySource());
  } catch { /* not yet implemented */ }

  try {
    const { PasteSource } = await import('../src/parsers/paste-source.js');
