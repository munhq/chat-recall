...
const STATUS_FIXTURE = {
  ...
  projects: {
    ...
    // Two projects under inco → inco becomes a folder with two leaves.
    '/home/user/code/inco/starter': 1,
    '/home/user/code/inco/platform': 2,
  },
};
...
test.describe('Project sidebar tree', () => {
  ...
  test('collapses shared /home/user/code prefix and renders categories as top level', async ({ page }) => {
    // `personal` and `inco` sit directly under the shared prefix and
    ...
    expect(paths).toContain('/home/user/code/inco');
  ...
  test('children sort folders before leaves, alphabetic within each group', async ({ page }) => {
    // Top level: inco and personal are both folders → alphabetic order.
