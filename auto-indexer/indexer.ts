...
import { getDiaryDir, getIndexDir } from '../src/core/paths.js';
import { SummaryGenerator, reapShellChildren } from '../src/core/summary-generator.js';
import { loadSettings, applySettingsToEnv } from '../src/core/settings.js';
import { parseSessionFile, getAllSessions } from '../src/parsers/session.js';
...
import { createKnowledgeGraph } from '../src/core/store/knowledge-graph.js';
import { statSync, existsSync, readFileSync } from 'fs';
import { dirname, join, basename } from 'path';

import { claudeBackend, geminiBackend, opencodeBackend, codexBackend } from '../src/core/backends/index.js';
import { isQuotaError, parseQuotaRetryMs } from '../src/core/quota-detect.js';
...
async function computeAndPersistSession(
  ...
  const { replaySessionAny, extractTurnsAny } = await import('../src/core/session-multi-tool.js');
  const { getBackendForId } = await import('../src/core/tool-backend.js');
  await import('../src/core/backends/index.js'); // side-effect: register
  ...
  try {
    ...
    if (missing.includes('secrets')) {
      ...
      const { scanSessionForSecrets, isSecretScannerAvailable } = await import('../src/core/secret-scanner.js');
      if (isSecretScannerAvailable()) {
        const { createStore } = await import('../src/core/store/index.js');
        const store = await createStore();
        try {
