...
describe('resolveWorkspaceId', () => {
  function incoWorkspace() {
    // /<root>/inco/{a,b,c} — 3 git repos promotes `inco` to a workspace.
    const names = ['repo-a', 'repo-b', 'repo-c'];
    for (const n of names) gitInit(mk('inco', n), `git@github.com:inco/${n}.git`);
    return names;
  }

  test('repo directly under workspace → ws:inco', () => {
    incoWorkspace();
    expect(resolveWorkspaceId(join(root, 'inco', 'repo-a'))).toBe('ws:inco');
  });

  test('DEEP subdir inside a repo still resolves to ws:inco (CWD-independent)', () => {
    incoWorkspace();
    const deep = mk('inco', 'repo-a', 'pkg', 'sub', 'dir');
    expect(resolveWorkspaceId(deep)).toBe('ws:inco');
  });

  test('every repo + subdir in the workspace agrees on the same id', () => {
    incoWorkspace();
    const ids = [
      resolveWorkspaceId(join(root, 'inco', 'repo-a')),
      resolveWorkspaceId(mk('inco', 'repo-b', 'deep', 'path')),
      resolveWorkspaceId(join(root, 'inco', 'repo-c')),
    ];
    expect(new Set(ids)).toEqual(new Set(['ws:inco']));
  });
