...
export function licenceSerialMail(to: string, serial: string, interval: 'month' | 'year'): Mail {
  const text = [
    ...
    '',
    'That unlocks COLLABORATION on your own infrastructure: shared project history,',
    'the team task board, per-member activity, and skills and MCP config distributed',
    'to every machine. Running chat-recall for one person is free and already',
    'includes sync, code findings, secret monitoring and analytics — this licence',
    'adds the second person, not the product.',
    '',
