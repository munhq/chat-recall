...
FOR tn IN SELECT tenant FROM tenants LOOP
  ...
  -- ── 3. Project ids orphaned by the munhq → munhq GitHub org rename ───────
  ...
  --
  -- Scope is narrow on purpose — only git:github.com/munhq/* → munhq/*,
  -- verified against the working tree where every affected repo's origin now
  -- points at munhq/<same-name>. acme-corp, darkkraft, maintainer and third-party
  ...
  -- `path:…/argocd-apps-munhq` project) is never caught.
  ...
  DELETE FROM code_findings f
   WHERE f.project_id LIKE 'git:github.com/munhq/%'
     AND EXISTS (SELECT 1 FROM code_projects p
                  WHERE p.project_id = replace(f.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/'));

  DELETE FROM code_hotspots h
   WHERE h.project_id LIKE 'git:github.com/munhq/%'
     AND EXISTS (SELECT 1 FROM code_projects p
                  WHERE p.project_id = replace(h.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/'));

  DELETE FROM code_actions a
   WHERE a.project_id LIKE 'git:github.com/munhq/%'
     AND EXISTS (SELECT 1 FROM code_projects p
                  WHERE p.project_id = replace(a.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/'));

  DELETE FROM code_projects s
   WHERE s.project_id LIKE 'git:github.com/munhq/%'
     AND EXISTS (SELECT 1 FROM code_projects p
                  WHERE p.project_id = replace(s.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/'));
