-- 0007: consolidate project ids orphaned by the munhq → munhq GitHub org rename.
--
-- WHY THIS IS NEEDED
--
...
-- changes it. When the `munhq` org became `munhq`, every affected repo therefore
...
--
-- SCOPE — deliberately narrow. Only `git:github.com/munhq/*` is rewritten, to
...
-- data (acme-corp, darkkraft, maintainer, and third-party clones) are NOT touched —
...
UPDATE memory_metadata
   SET project_id = replace(project_id, 'git:github.com/munhq/', 'git:github.com/munhq/')
 WHERE project_id LIKE 'git:github.com/munhq/%';
...
DELETE FROM code_findings f
 WHERE f.project_id LIKE 'git:github.com/munhq/%'
   AND EXISTS (
     SELECT 1 FROM code_projects p
      WHERE p.tenant = f.tenant
        AND p.project_id = replace(f.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/')
   );

DELETE FROM code_hotspots h
 WHERE h.project_id LIKE 'git:github.com/munhq/%'
   AND EXISTS (
     SELECT 1 FROM code_projects p
      WHERE p.tenant = h.tenant
        AND p.project_id = replace(h.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/')
   );

DELETE FROM code_actions a
 WHERE a.project_id LIKE 'git:github.com/munhq/%'
   AND EXISTS (
     SELECT 1 FROM code_projects p
      WHERE p.tenant = a.tenant
        AND p.project_id = replace(a.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/')
   );

DELETE FROM code_projects s
 WHERE s.project_id LIKE 'git:github.com/munhq/%'
   AND EXISTS (
     SELECT 1 FROM code_projects p
      WHERE p.tenant = s.tenant
        AND p.project_id = replace(s.project_id, 'git:github.com/munhq/', 'git:github.com/munhq/')
   );
