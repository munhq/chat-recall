-- Account-scope decisions that no caller ever asked for. RENUMBERED FROM 0014.
--
-- 0014 ran in production and reported "retired 0" against a database with 16
-- matching rows, then the ledger recorded it as applied, which makes a silent
-- no-op permanent. It set app.tenant and not app.viewer; author_visibility on
-- kg_triples is RESTRICTIVE, so with no viewer the UPDATE saw almost nothing.
-- A renamed file is the only way past a ledger entry — the same reason 0012
-- exists.
--
--
...
BEGIN
  ...
  FOR tn IN SELECT tenant FROM tenants LOOP
    ...
    -- BOTH GUCs. 0014 set only the tenant and matched ZERO rows on a database
    -- holding 16 of them, because author_visibility on kg_triples is
    -- RESTRICTIVE and an unset viewer admits almost nothing. tenantQuery sets
    -- two; setting one is not "exactly as tenantQuery does".
