...
function makeSqlite(): () => Promise<Harness> {
  return async () => {
    ...
    return { store, teardown: async () => { await store.close(); await removeTestDir(tmp); } };
  };
}
...
function behaviorSuite(label: string, setup: () => Promise<Harness>) {
  describe(`StorageDriver behavior — ${label}`, () => {
    ...
    test('querySessionIndex project filter: bare term = substring, typed id = exact', async () => {
      ...
      await store.setItem({ ...item('s2', 'session'), projectPath: '/home/u/code/acme-monorepo' });
      ...
      const ci = await store.querySessionIndex({ limit: 10, offset: 0, projectIdFilter: 'ACME', includeUntracked: true });
...
describe('SqliteStore — wiring parity (wrapper === inner)', () => {
  ...
  afterEach(async () => { await store.close(); await removeTestDir(tmp); });
...
describe('SqliteMetadataCache', () => {
  ...
  afterEach(async () => { await mc.close(); await removeTestDir(tmp); });
...
describe('SqliteOutcomeCache', () => {
  ...
  afterEach(async () => { await oc.close(); await removeTestDir(tmp); });
...
describe('SqliteKnowledgeGraph', () => {
  ...
  afterEach(async () => { await kg.close(); await removeTestDir(tmp); });
...
describe('FileWal', () => {
  ...
  afterEach(async () => { await removeTestDir(tmp); });
...
describe('FileDiary', () => {
  ...
  afterEach(async () => { if (prev === undefined) delete process.env.CHAT_RECALL_DATA_DIR; else process.env.CHAT_RECALL_DATA_DIR = prev; await removeTestDir(tmp); });
...
describe('createStore factory + resolveBackend', () => {
  test('explicit sqlite backend → SqliteStore', async () => {
    ...
    await s.close(); await removeTestDir(tmp);
  });
