...
`mcp__chat-recall__recall_security_rules` — list the tenant's custom
...
- `mcp__chat-recall__recall_task_create` — file the rotation as a card, so it
  survives the end of this session. See [chat-recall-tasks].
- `mcp__chat-recall__recall_recommendations` (scope `account`) — the scanner's
