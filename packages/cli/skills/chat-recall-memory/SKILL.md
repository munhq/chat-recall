...
- `mcp__chat-recall__recall_decisions` — the decision register, cascade already
...
- `mcp__chat-recall__recall_diary_write` — narrative notes ("what I learned /
  what matters") keyed per agent, across sessions.
- `mcp__chat-recall__recall_set` — small structured state (current PR url,
