...
/** The ChatGPT-app copy: same skills, tool names unprefixed. Generated by
 *  scripts/build-plugin.mjs; see the chatgpt-parity block at the bottom. */
const chatgptSkillsDir = join(here, '..', '..', '..', 'chatgpt-plugin', 'skills');
...
/**
 * The ChatGPT app copy.
 *
 * ChatGPT exposes this server's tools under their BARE names — OpenAI's scan of
 * https://chatrecall.dev/mcp lists `recall_search`, not
 * `mcp__chat-recall__recall_search`, which is Claude Code's namespacing. A skill
 * that names the prefixed form there points an agent at a tool that does not
 * exist, so this mirror strips the prefix and these tests pin that it is a pure
 * rename of the same files and nothing else.
 */
describe('chatgpt-plugin parity', () => {
  const chatgptSkills = readdirSync(chatgptSkillsDir, { withFileTypes: true })
    .filter((e) => e.isDirectory() && existsSync(join(chatgptSkillsDir, e.name, 'SKILL.md')))
    .map((e) => e.name)
    .sort();

  test('ships the same set of skills as the CLI', () => {
    expect(chatgptSkills).toEqual(skills.map((s) => s.name).sort());
  });

  test.each(skills.map((s) => s.name))('%s differs from the CLI copy ONLY by the tool prefix', (name) => {
    const file = join(chatgptSkillsDir, name, 'SKILL.md');
    expect(existsSync(file), `chatgpt-plugin/skills/${name}/SKILL.md is missing — run scripts/build-plugin.mjs`).toBe(true);
    const source = skills.find((s) => s.name === name)!.text;
    expect(
      readFileSync(file, 'utf-8'),
      `chatgpt-plugin/skills/${name}/SKILL.md drifted — run scripts/build-plugin.mjs`,
    ).toBe(source.split(MCP_PREFIX).join(''));
  });

  test('carries no Claude-namespaced tool name', () => {
    for (const n of chatgptSkills) {
      expect(readFileSync(join(chatgptSkillsDir, n, 'SKILL.md'), 'utf-8')).not.toContain(MCP_PREFIX);
    }
  });

  test('still names every registered tool', () => {
    const text = chatgptSkills
      .map((n) => readFileSync(join(chatgptSkillsDir, n, 'SKILL.md'), 'utf-8'))
      .join('\n');
    expect(defined.filter((t) => !text.includes(t))).toEqual([]);
  });
});
