...
- If a repo isn't indexed yet, `recall_code_index` (path) runs
  the analyzer and syncs findings.

## Findings, ranked (no codeindex binary required)

- `recall_claude_suggestions` — every finding that turns into
...
- `recall_improvements` — everything else, ranked most urgent
...
- `recall_recommendation_apply` — apply one: add its rule to
  ...
  destructively.
- `recall_recommendation_dismiss` — say no for THIS project,
...
- `recall_tasks` — the shared task board: status, assignee,
  project. This is the answer to "what are my open tasks".
- `recall_task_create` — create a task, optionally assigned to
  a teammate and attached to a project. Returns the new task id.
- `recall_task_update` — change status, reassign, rename or
...
- `recall_team_activity` — per-teammate × per-project rollup
  (session counts, last activity) for "what is the team working on".
- `recall_shares` — which projects are shared into the team
