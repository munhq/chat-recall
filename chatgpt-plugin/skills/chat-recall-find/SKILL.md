...
- `recall_search` — search SESSIONS. Free-text query.
...
- `recall_memory_search` — search ALL memory types, not just
...
- `recall_user_prompts` — search only what the USER actually
  typed (assistant output stripped). Best for "what did I ask about X",
  "what was I requesting yesterday".
- `recall_subagent_search` — search inside SUBAGENT
...
- `recall_redundant_files` — before creating a file, check
...
- `recall_show` — read the session slice (`around_line`,
...
- `recall_summary` — AI summary + outcome.
- `recall_memory_item` — read ONE item in full (or its links)
