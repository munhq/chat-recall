...
`recall_smart_resume` returns a structured bundle: what was
...
1. Get the id. `recall_recent` (`project_filter`,
   `since_hours`) lists candidates newest-first — the right call for a plain
   "continue". Use `recall_search` with
...
- `recall_wake_up` — identity blurb + high-importance
...
- `recall_project_context` — when the user is clearly working in
